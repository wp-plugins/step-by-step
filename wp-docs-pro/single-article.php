...
<div role="navigation" class="signer-header">
	<div class="container">
		<div class="nav-menu"><ul style="float: left; word-wrap: break-word;width: 500px! important;margin-top: 1px;"><li>
		<?php 
      ...
      wp_nav_menu( array( 'theme_location' => 'primary', 'menu_class' => 'nav-menu','container_class' => 'nav-menu' ) );
		}
		?>
	</div>
</div>
	...
	<?php while ( have_posts() ) : the_post(); ?>
		<article id="post-<?php the_ID(); ?>" class="article type-article status-publish has-post-thumbnail">
			<div class="entry-content" style=" border: medium none !important;margin: 0 !important;max-width: 100% !important;padding: 0 !important;width: 100% !important;">
				...
				$string .='<!-- Step-by-Step Plugin by Kyle m. Brown //-->
				...
				//	$string .='<span class="doc_title">Steps</span>';
				$num = 1;	
				...
				if(!empty($article_steps_meta["step_listView"][0]) && $article_steps_meta["step_listView"][0] == "yes"){
					...
					$string .='<dd class="dd-list-view"><h4 class="display-hlist">'.$article_steps_meta["step"][$i].'</h4><p class="add-popup">'.str_replace('<a " ','<a  class="thickbox " ',$article_steps_meta["step_title"][$i]).'<br><strong>'.substr($article_steps_meta["note"][$i],0,125).'</strong></p>';
					...
					$string .='<dd><div class="step-content  grid-container"><div class="step-image  grid-img">'.$imgString.'</div><div class="step-text grid-detail"><h4  class="display-hlist">'.$article_steps_meta["step"][$i].' </h4><p class="add-popup">'.str_replace('<a " ','<a  class="thickbox"  ',$article_steps_meta["step_title"][$i]).'<br><strong>'.substr($article_steps_meta["note"][$i],0,125).'</strong></div><div style="clear:both;"></div>';
					$string .='</dd>';
				}
				$num++;
			} 
